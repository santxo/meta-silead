...
static int gslx680_load_fw(struct i2c_client *client)
{
	struct gslx680_ts_data *data = i2c_get_clientdata(client);
	struct device *dev = &client->dev;
	...
	int error;
	const struct gslx680_fw_data GSLX680_FW[] = {
	...
	unsigned int line = 0;
	unsigned int lines = sizeof(GSLX680_FW) / sizeof(GSLX680_FW[0]);
	char buf[4] = {0, 0, 0, 0};

	//	fw_data = (struct gslx680_fw_data *)GSLX680_FW[];
	
	for (line = 0; line < lines; line++) {
		buf[0] = (char)(GSLX680_FW[line].val & 0x000000ff);
		buf[1] = (char)(GSLX680_FW[line].val & 0x0000ff00) >> 8;
		buf[2] = (char)(GSLX680_FW[line].val & 0x00ff0000) >> 16;
		buf[3] = (char)(GSLX680_FW[line].val & 0xff000000) >> 24;

		error = i2c_smbus_write_i2c_block_data(client, GSLX680_FW[line].offset, 4, buf);
		if (error) {
			dev_err(dev, "I f'd whilst loading fw...");
			break;
		}
	}
